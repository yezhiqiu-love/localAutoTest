<?xml version="1.0" encoding="UTF-8" ?>
<!-- Written by James Zhang -->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
<xsl:include href="fitnesseResultsToHtml.xsl" />

<xsl:template name="css_and_javascript">
    <style type="text/css">
        <xsl:value-of select="document('files/css/fitnesse_base.css')" disable-output-escaping="yes" />
    </style>
    <script language="JavaScript" type="text/javascript">
        <xsl:value-of select="document('files/javascript/fitnesse.js')" disable-output-escaping="yes" />
        <xsl:value-of select="document('files/javascript/fitnesseResults.js')" disable-output-escaping="yes" />
        <![CDATA[
if (isIE()) {
    collapsableOpenImg = "http://fitnesse.org/files/images/collapsableOpen.gif";
    collapsableClosedImg = "http://fitnesse.org/files/images/collapsableClosed.gif";
} else {
    collapsableOpenImg = "data:image/gif;base64,";
    collapsableClosedImg = "data:image/gif;base64,";
}
        ]]>
    </script>
</xsl:template>

</xsl:stylesheet>

